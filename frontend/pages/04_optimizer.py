...
def safe_df(data) -> pd.DataFrame:
    """Convert any API response shape into a DataFrame safely."""
    if data is None:
        return pd.DataFrame()
...
# Column name aliases — handles both DB names and live optimizer names
SQ_COL  = None   # resolved below
OPT_COL = None


def resolve_cols(df: pd.DataFrame):
    """Return the actual sq/opt persondays column names present in df."""
...
min_frac     = st.sidebar.slider("Min alloc per district", 0.1, 0.8, 0.4, 0.05,
    help="Floor: min fraction of current budget per district")
max_frac     = st.sidebar.slider("Max alloc per district", 1.1, 4.0, 2.5, 0.1,
    help="Cap: max multiple of current budget per district")
...
if not df_pre.empty and not run_btn:
    ...
    if sq_col and opt_col:
        ...
        st.warning(f"Unexpected columns in optimizer data: {list(df_pre.columns)}")

    st.markdown("---")
    _render_charts(df_pre, scope)

elif df_pre.empty and not run_btn:
    st.info("No pre-computed results found. Click **Run Optimizer** to generate.")
...
if run_btn:
    ...
    if result:
        ...
        if not df_live.empty:
            # Live results use sq_persondays / opt_persondays too
            _render_charts(df_live, result.get("scope", scope))


def _render_charts(df: pd.DataFrame, scope: str):
    """Render allocation comparison and efficiency scatter."""

    # Determine sq/opt col names
    ...
    with col_b:
        ...
        fig2 = px.scatter(
            ...
            labels={
                "persondays_per_lakh": "Efficiency (PD per Rs. lakh)",
    ...
    show_cols = [c for c in ["state", "district", "budget_allocated_lakhs",
                              "optimized_budget", "budget_change_pct",
                              gain_col] if c and c in df.columns]
